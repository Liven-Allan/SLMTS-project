...
class User(models.Model):
    ...
    # Required fields for Django auth system
...
class Order(models.Model):
    ...
    def calculate_totals(self):
        ...
        # Count actual individual items instead of order item quantities
        total_individual_items = 0
        for order_item in order_items:
            individual_items_count = order_item.individual_items.count()
            if individual_items_count > 0:
                total_individual_items += individual_items_count
            else:
                # Fallback to quantity if no individual items (for legacy orders)
                total_individual_items += int(order_item.quantity)
        
        self.items = total_individual_items
        self.save()
...
# =============================================================================
# 6. INDIVIDUAL ITEM MODEL (Customer-specified items)
# =============================================================================

class IndividualItem(models.Model):
    """
    Individual items within an order item (service)
    Allows customers to specify names for each item
    """
    order_item = models.ForeignKey(OrderItem, on_delete=models.CASCADE, related_name='individual_items')
    item_name = models.CharField(max_length=200)  # Customer-specified name like "Blue Shirt", "Favorite Jeans"
    item_type = models.CharField(max_length=100, blank=True)  # Auto-detected or manual: "shirt", "pants", etc.
    
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    
    def __str__(self):
        return f"{self.item_name} ({self.order_item.service.name})"
...
# =============================================================================
# 7. ORDER TIMELINE MODEL (From CustomerDashboard)
# =============================================================================
...
class RFIDTag(models.Model):
    ...
    Used in staff dashboard for item scanning and verification
    """
    STATUS_CHOICES = [
        ('pending', 'Pending'),
        ('scanned', 'Scanned'),
    ...
    individual_item = models.OneToOneField('IndividualItem', on_delete=models.CASCADE, related_name='rfid_tag', null=True, blank=True)
    item_description = models.CharField(max_length=200, blank=True)  # Fallback for legacy data
    item_type = models.CharField(max_length=100, blank=True)  # "shirt", "pants", "towel", etc.
    status = models.CharField(max_length=15, choices=STATUS_CHOICES, default='pending')
    
    @property
    def display_name(self):
        """Get the display name for this item"""
        if self.individual_item:
            return self.individual_item.item_name
        return self.item_description or f"Item {self.tag_id}"
    
    @property
    def service_name(self):
        """Get the service name for this item"""
        if self.individual_item:
            return self.individual_item.order_item.service.name
        return "Unknown Service"
    
    # Staff verification
    verified_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='verified_tags')
    ...
    def generate_tag_id(self):
        """Generate unique tag ID based on order"""
        if not self.tag_id:
            # Get the count of existing tags for this order
            existing_count = RFIDTag.objects.filter(order=self.order).count()
            self.tag_id = f"RF{self.order.id:03d}{existing_count + 1:03d}"
    
    def save(self, *args, **kwargs):
        if not self.tag_id:
            self.generate_tag_id()
        super().save(*args, **kwargs)
    
    def __str__(self):
        return f"{self.tag_id} - {self.display_name}"


# =============================================================================
# SIGNALS FOR AUTOMATIC RFID TAG CREATION
# =============================================================================

from django.db.models.signals import post_save
from django.dispatch import receiver

@receiver(post_save, sender=IndividualItem)
def create_rfid_tag_for_item(sender, instance, created, **kwargs):
    """
    Automatically create an RFID tag when an IndividualItem is created
    """
    if created:
        RFIDTag.objects.create(
            order=instance.order_item.order,
            individual_item=instance,
            item_type=instance.item_type or 'garment'
        )
...
class Delivery(models.Model):
    ...
    # Completion details
    completed_at = models.DateTimeField(null=True, blank=True)
    signature_image = models.CharField(max_length=500, null=True, blank=True)  # Temporary: store image path/URL
    photo_confirmation = models.CharField(max_length=500, null=True, blank=True)  # Temporary: store image path/URL
...
"""
# =============================================================================
# CUSTOM AUTH TOKEN MODEL
# =============================================================================

class AuthToken(models.Model):
    """
    Custom authentication token for API access
    Works with our custom User model
    """
