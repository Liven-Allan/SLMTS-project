...
![CityVille Laundromat](SLMTS-frontend/public/laundry.png)
...
The system includes several management commands to populate the database with sample data (run from `SLMTS-backend/` directory):
...
After running migrations, you can create users through the registration system or use the Django admin (run from `SLMTS-backend/` directory):
...
- **API Base URL**: Configure in `SLMTS-frontend/src/services/api/config.ts`
- **Environment Variables**: Set up in `SLMTS-frontend/.env` file
- **CORS Settings**: Configured for development

### Backend Configuration
- **Database**: SQLite (development) - configure in `SLMTS-backend/SLMTS_project/settings.py`
...
### Backend Deployment
1. Set `DEBUG = False` in `SLMTS-backend/SLMTS_project/settings.py`
